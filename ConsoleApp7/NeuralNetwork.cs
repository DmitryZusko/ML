using Newtonsoft.Json;

namespace NeuralNetwork
{

	public class NeuralNetwork
	{
		public Topology Topology { get; }
		public List<Layer> Layers { get; }

		public NeuralNetwork(Topology topology)
		{
			Topology = topology;

			Layers = new List<Layer>();

			CreateInputLayer();
			CreateHiddenLayers();
			CreateOutputLayer();
			LoadFromDBWeights();
		}
		//переганяємо дані по нейронам
		public Neuron FeedForward(params double[] inputSignals)
		{
			SendSignalsToInputNeurons(inputSignals);
			FeedForwardAllLayersAfterInput();

			if (Topology.OutputCount == 1)
			{
				return Layers.Last().Neurons[0];
			}
			else
			{
				return Layers.Last().Neurons.OrderByDescending(n => n.Output).First();
			}
		}

		public double Learn(List<Tuple<double, double[]>> dataset, int epoch, int accuracy)
		{
			var error = 0.0;
			var avgEpochErr = 0.0;
			var maxEpochErr = 0.0;
			var epochErrCount = 0;
			using (FileStream fse = new FileStream("errors.txt", FileMode.Create))
			{
				using (StreamWriter writer = new StreamWriter(fse))
				{
					//writer.WriteLine(error);


					for (int i = 0; i < epoch; i++)
					{
						error = 0.0;
						avgEpochErr = 0.0;
						maxEpochErr = 0.0;
						epochErrCount = 0;
						foreach (var data in dataset)
						{
							error = Backpropagation(accuracy, data.Item1, data.Item2);
							//writer.WriteLine(error);
							avgEpochErr += error;
							if (error - maxEpochErr > 0d)
								maxEpochErr = error;
							if (Math.Round(error, 1) == 0)
								epochErrCount++;
						}
						//error = error/dataset.Count;
						avgEpochErr = avgEpochErr / dataset.Count;
						writer.WriteLine("epoch	" + i + "	| " + epochErrCount + " |	" + avgEpochErr + "	" + maxEpochErr);
					}
				}

			}
			var result = error;/// epoch;
			return result;
		}


		//метод обратного распространения ошибки
		private double Backpropagation(int accuracy, double exprected, params double[] inputs)
		{
			var actual = FeedForward(inputs).Output;
			// для output layer помилка вираховується трішки по іншому(err = actual - expected)
			var difference = actual - exprected;
			//if (Math.Round(difference, accuracy) == 0)
			//	return difference * difference;

			foreach (var neuron in Layers.Last().Neurons)
			{
				neuron.Learn(difference, Topology.LearningRate);
			}
			//////////////////////////////////////////////////////////////////////
            for (int j = Layers.Count - 2; j >= 0; j--)
            {
                var layer = Layers[j];
                var previousLayer = Layers[j + 1];

                for (int i = 0; i < layer.Count; i++)
                {
                    var neuron = layer.Neurons[i];

                    for (int k = 0; k < previousLayer.Count; k++)
                    {
                        var previousNeuron = previousLayer.Neurons[k];
                        var error = previousNeuron.Weights[i] * previousNeuron.Delta;
                        neuron.Learn(error, Topology.LearningRate);
                    }
                }
            }
			///////////////////////////////////////////////////////////////////////
   //         var err = 0d;
			//for (int j = Layers.Count - 2; j >= 0; j--)
			//{
			//	var layer = Layers[j];
			//	var previousLayer = Layers[j + 1];

			//	for (int ner = 0; ner < layer.Neurons.Count; ner++)
			//	{
			//		err = 0d;
			//		for (int pln = 0; pln < previousLayer.Neurons.Count; pln++)
			//		{
			//			err += previousLayer.Neurons[pln].Delta * previousLayer.Neurons[pln].Weights[ner];
			//		}
			//		layer.Neurons[ner].Learn(err, Topology.LearningRate);

			//	}

			//}
			//////////////////////////////////////////////////////////////////////////

			var result = difference * difference;
			return result;
		}
		/*private void BackpropagationUnsupervised(params double[] inputs)
		{
			var output = FeedForward(inputs).Output;

			
			for (int j = Layers.Count - 2; j >= 0; j--)
			{
				var layer = Layers[j];
				var nextLayer = Layers[j + 1];

				for (int i = 0; i < layer.Count; i++)
				{
					var neuron = layer.Neurons[i];
					var error = 0.0;

					for (int k = 0; k < nextLayer.Count; k++)
					{
						var nextNeuron = nextLayer.Neurons[k];
						error += nextNeuron.Weights[i] * nextNeuron.Delta;
					}

					neuron.Learn(error, Topology.LearningRate);
				}
			}
		}*/


		private void FeedForwardAllLayersAfterInput()
		{
			for (int i = 1; i < Layers.Count; i++)
			{
				var layer = Layers[i];
				var previousLayerSingals = Layers[i - 1].GetSignals();

				foreach (var neuron in layer.Neurons)
				{
					neuron.FeedForward(previousLayerSingals);
				}
			}
		}

		private void SendSignalsToInputNeurons(params double[] inputSignals)
		{
			for (int i = 0; i < inputSignals.Length; i++)
			{
				var signal = new List<double>() { inputSignals[i] };
				var neuron = Layers[0].Neurons[i];

				neuron.FeedForward(signal);//відправляємо дані на input, так як ми будемо по черзі проходити по кожному слою
			}
		}

		private void CreateOutputLayer()
		{
			var outputNeurons = new List<Neuron>();
			var lastLayer = Layers.Last();
			for (int i = 0; i < Topology.OutputCount; i++)
			{
				var neuron = new Neuron(lastLayer.Count, NeuronType.Output);
				outputNeurons.Add(neuron);
			}
			var outputLayer = new Layer(outputNeurons, NeuronType.Output);
			Layers.Add(outputLayer);
		}

		private void CreateHiddenLayers()
		{
			for (int j = 0; j < Topology.HiddenLayers.Count; j++)
			{
				var hiddenNeurons = new List<Neuron>();
				var lastLayer = Layers.Last();
				for (int i = 0; i < Topology.HiddenLayers[j]; i++)
				{
					var neuron = new Neuron(lastLayer.Count);
					hiddenNeurons.Add(neuron);
				}
				var hiddenLayer = new Layer(hiddenNeurons);
				Layers.Add(hiddenLayer);
			}
		}

		private void CreateInputLayer()
		{
			var inputNeurons = new List<Neuron>();
			for (int i = 0; i < Topology.InputCount; i++)
			{
				var neuron = new Neuron(1, NeuronType.Input);
				inputNeurons.Add(neuron);
			}
			var inputLayer = new Layer(inputNeurons, NeuronType.Input);
			Layers.Add(inputLayer);
		}

		public void LoadFromDBWeights()
		{
			string jWeights = "";

			using (FileStream fsr = new FileStream("weights.json", FileMode.Open))
			{
				using (StreamReader sr = new StreamReader(fsr))
				{
					jWeights = sr.ReadToEnd();
				}
			}
			if (!string.IsNullOrEmpty(jWeights))
			{
				List<List<List<double>>> weights = JsonConvert.DeserializeObject<List<List<List<double>>>>(jWeights);
				if (Layers.Count == weights.Count)
                {
					for (int l = 0; l < weights.Count; l++)
                    {
						List<List<double>> neuro = weights[l];
						var layer = Layers[l];
						if (neuro.Count == layer.Neurons.Count)
						{
							for (var i = 0; i < neuro.Count; i++)
							{
								if (layer.Neurons[i].Weights.Count == neuro[i].Count)
									layer.Neurons[i].Weights = neuro[i];
							}
						}
					}
                }



				//foreach (var layer in Layers)
				//{
				//	if (layer.Type == NeuronType.Output)
				//	{
				//		if (weights.Count >= 3)
				//		{
				//			List<List<double>> neuro = weights[2];
				//			if (neuro.Count == layer.Neurons.Count)
				//			{
				//				for (var i = 0; i < neuro.Count; i++)
				//				{
				//					if (layer.Neurons[i].Weights.Count == neuro[i].Count)
				//						layer.Neurons[i].Weights = neuro[i];
				//				}
				//			}
				//		}
				//		//layer.Neurons = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Neuron>>("GenWeightsJson"); // треба зчитати останній масив 
				//	}
				//	if (layer.Type == NeuronType.Normal)
				//	{
				//		if (weights.Count >= 3)
				//		{
				//			List<List<double>> neuro = weights[1];
				//			if (neuro.Count == layer.Neurons.Count)
				//			{
				//				for (var i = 0; i < neuro.Count; i++)
				//				{
				//					if (layer.Neurons[i].Weights.Count == neuro[i].Count)
				//						layer.Neurons[i].Weights = neuro[i];
				//				}
				//			}
				//		}
				//		//layer.Neurons = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Neuron>>("GenWeightsJson");//зчитуємо всі крім останнього 
				//	}
				//	if (layer.Type == NeuronType.Input)
				//	{
				//		if (weights.Count >= 3)
				//		{
				//			List<List<double>> neuro = weights[0];
				//			if (neuro.Count == layer.Neurons.Count)
				//			{
				//				for (var i = 0; i < neuro.Count; i++)
				//				{
				//					if (layer.Neurons[i].Weights.Count == neuro[i].Count)
				//						layer.Neurons[i].Weights = neuro[i];
				//				}
				//			}
				//		}
				//		//layer.Neurons = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Neuron>>("GenWeightsJson"); // треба зчитати останній масив 
				//	}
				//	//layer.Neurons = Newtonsoft.Json.JsonConvert.DeserializeObject<List<Neuron>>("GenWeightsJson");
				//}
			}
					
		}
	}
}